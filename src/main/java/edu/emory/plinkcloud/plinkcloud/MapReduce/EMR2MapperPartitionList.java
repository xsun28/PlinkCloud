/*using interval sampler for already sorted original files*/
...
public class EMR2MapperPartitionList extends Configured implements Tool {
...
public static class PartitionReducer extends Reducer<LongWritable, IntWritable, LongWritable, NullWritable> {
	...
	System.out.println("chrmnum is "+chrmnum);
...
outputStr+=(count+":"+refnu+refnu+"\t");
	...
	outputStr+=(count+":"+alt[count-1]+alt[count-1]+"\t");
	}
}
...
mos.write(NullWritable.get(),result, "biresult");
...
mos.write(NullWritable.get(), frequency, "bifreq");
}
/*else{
...
public static Configuration getSecondJobConf( Tool tool, Configuration conf, String chrm, Path inputPath, Path outputPath, String partitionlst, double samplerate, int indno)
   ...
   ControlledJob []jobs;
	...
	if(leftchrmno==0) {
		...
		for(int i=0; i<chromsno; i++){
			...
			jobs[i]=new ControlledJob(getSecondJobConf(this, conf,chroms[i].substring(3),new Path(chrmFiles[i]),new Path(chrmResults[i]),args[1], sampleRate, indno ));
		jc.addJob(jobs[i]);
		}
...
public static void main(String[] args) throws Exception{
	int code=ToolRunner.run(new EMR2MapperPartitionList(), args);
	System.exit(code);
}
